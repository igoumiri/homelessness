...
The analysis relies on an integrated dataset constructed from several public sources. The unit of analysis is the Continuum of Care (CoC). The key variables are the number of people experiencing homelessness categorized by year and by age group, the total population, the average housing cost, and geographic attributes.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        Actual 65+ unhoused population size (left) and predicted 65+ unhoused population when controlling for population, spatial lag, and housing cost (right) in California CoCs in 2023 (log scale).
    }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{
        \label{fig:maps:prop}
        65+ unhoused proportion relative to total population (left) and relative discrepancy of leave-one-out prediction of homelessness counts vs actual counts for each CoC (right).
    }
\end{figure}
